...
async def main(ip: str):
    async with LinWave(ip) as lw:
        ...
        await lw.set_filter(channel=0, highpass=0, lowpass=0)  # 100-450 kHz bandpass
        ...
        await lw.set_status_interval(channel=0, seconds=0)  # generate status data every 2 seconds
        await lw.set_threshold(channel=0, microvolts=1_00)  # 1000 µV = 60 dB(AE) 40
        ...
        async for record in merge_ae_tr_records(lw.acquire()):
            ...
            print(record.data)
            i=i+1
            #np.save(r"C:\Users\djeri\OneDrive\Desktop\test", record)
            np.savetxt(rf"C:\Users\djeri\OneDrive\Desktop\data_tube\all\data{i}.csv", record.data, delimiter=",")
